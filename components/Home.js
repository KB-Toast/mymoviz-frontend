...
function Home() {
  ...
  // Movies list
  /* old code to replace with useEffect
  const moviesData = [
  ...
  */
  ...
  useEffect(() => {
    fetch('https://mymoviz-backend-seven-psi.vercel.app/movies')
      ...
      .then((data) => {
        ...
        data.results.map((singleMovie) => {
          ...
          newMovie.voteAverage = singleMovie.vote_average;
          newMovie.voteCount = singleMovie.vote_count;
          newMovie.overview = singleMovie.overview;
          newMovie.movieId = singleMovie.id;
